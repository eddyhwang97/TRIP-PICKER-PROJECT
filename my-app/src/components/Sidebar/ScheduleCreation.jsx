...
export default function ScheduleCreation(props) {
  const { placesInfo, dailyTimeSlots } = props;
    ...
    if (placesInfo) {
      ...
      Object.entries(placesInfo).forEach(([category, places]) => {
        places.forEach((place) => {
          newSchedule.push({
            id: `${category}-${place.name}-${Math.random().toString(36).substr(2, 9)}`, // 고유 ID
  ...
  function handleOnDragEnd(result) {
    ...
    // 원래 schedule에서 제거
    const filteredSchedule = newSchedule.filter((item, idx) => !(item.date === sourceDate && draggedItems.indexOf(item) === source.index));
  ...
  return (
    <>
              ...
              <div key={date} className="day-group">
                ...
                {expandedDates.includes(date) && (
                  <Droppable draggableId={date}>
                    {(provided) => (
                      <ul className="place-list" {...provided.droppableProps} ref={provided.innerRef}>
                        {grouped[date].map((item, i) => (
                          ...
                          >
                            {(provided) => (
                              <li ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} className="place-item">
                                <img className="place-image" src={item.imageUrl} alt={item.place} />
                                <div className="place-info">
                                  <div className="place-name-category">
                                    <span className="place-name">{item.place}</span>
                                    {item.category && <span className={`place-category ${categoryColors[item.category]?.color || ""}`}>{categoryColors[item.category]?.title || item.category}</span>}
                                  </div>
      ...
      <SidebarButton step={4} setStep={props.setStep} />
    </>
  );
}
