  ...
  const clickEvent = {
    ...
    makeTrip: (city) => {
      ...
      const createTripId =
        usersTrip !== null
          ? usersTrip.length < 10
            ? `trip00${usersTrip.length + 1}`
            : usersTrip.length > 10
            ? `trip0${usersTrip.length + 1}`
            : `trip${usersTrip.length + 1}`
          : "trip001";
        ...
        <div className="dashboard-left">
          <section className="dashboard-left-top">
            <div className="dashboard-left-profile">
              <div
                className="dashboard-left-profile-image"
        ...
        <div className="dashboard-right">
          <section className="dashboard-right-top">
            ...
            <div className="dashboard-right-top-edit">
              {mode === "V" && (
                <span
                  className="dashboard-right-top-edit-button"
                  onClick={() => setMode("E")}
                >
                  편집
                </span>
              ...
              {mode === "E" && (
                <>
                  <span
                    className="dashboard-right-top-save-button"
                    onClick={() => setMode("V")}
                  >
                    저장
                  </span>
                  <span
                    className="dashboard-right-top-delete-button"
                    onClick={handleDeleteTripList}
                  >
                    삭제
                  </span>
                </>
          ...
          <section className="dashboard-right-bottom">
            <div className="dashboard-right-trip-item-box">
              <div className="dashboard-right-trip-item-list">
                <div
                  className="dashboard-right-trip-create-new-item"
                  onClick={setNewTrip}
                >
                  <div className="dashboard-right-trip-item-thumb-box">
                    <div className="dashboard-right-trip-item-thumb-image">
                      {newTrip ? (
                        <div className="main-intro-travle-location-search-box">
                          <div
                            className="main-intro-search-box"
                            onClick={clickEvent.showCityList}
                          >
                            <input
                              type="text"
                              className="main-intro-search-input"
                            ...
                            <ul>
                              {cityList.length > 0 ? (
                                cityList.map((city, idx) => (
                                  <li
                                    className="main-intro-city-list"
                                    key={idx}
                                    onClick={() => (
                                      (cityInfo.current = city),
                                      clickEvent.makeTrip(city)
                                    )}
                                  >
                                    ...
                                    <div className="main-intro-location-name">
                                      <span className="main-intro-city-name-kr">
                                        {city.name}
                                      </span>
                                      <span className="main-intro-city-name-en">
                                        {city.englishName}
                                      </span>
                                    </div>
                                  </li>
                                ...
                                <li className="main-intro-city-list no-result">
                                  검색 결과가 없습니다.
                                </li>
                              )}
                            </ul>
                    ...
                    <p className="dashboard-right-trip-item-thumb-title">
                      {"새로운 여행"}
                    </p>
                  </div>
                </div>
                {tripList.map((trip) => (
                  <DashBoardItem
                    getCurrentTripData={getCurrentTripData}
                    key={trip.id}
                    trip={trip}
                    mode={mode}
                    onCheck={handleCheck}
